...
class GummyCallback(TranslationRecognizerCallback):
    ...
    def on_event(
        ...
        if self.enable_translation and translation_result and translation_result.is_sentence_end:
            is_final = True
        ...
        if is_final:
            ...
            if self.enable_translation and self.target_language and translation_result:
                try:
                    target_translation = translation_result.get_translation(self.target_language)
                    ...
                    self.logger.warning(f"最终结果中未找到目标语言 '{self.target_language}' 的翻译，尝试使用转录。")
                except Exception as e:
                    self.logger.error(f"处理最终翻译结果时出错: {e}", exc_info=True)

            # 如果没有翻译文本，使用转录文本
            if text_to_send is None and transcription_result and transcription_result.text:
        ...
        # --- 处理中间结果 ---
            ...
            if self.intermediate_behavior == "show_typing":
                ...
                if text_to_send:
                    self.logger.debug(f"{log_prefix}: {text_to_send}")  # 使用 debug 级别避免刷屏
                # else: # 部分结果为空时无需记录
...
def create_gummy_recognizer(
    ...
    logger.debug(f"Dashscope Gummy Recognizer (模型: {model}) 初始化完成。")  # Changed to DEBUG
