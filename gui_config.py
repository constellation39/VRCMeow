...
def create_dashscope_controls(
    ...
    model_options = []
    selected_model_value = stt_conf.get(
        "selected_model", None
    )  # Get configured selected model

    if not stt_models_config:
        logger.warning(
            "No STT models defined in config 'dashscope.stt.models'. Dropdown will be empty."
        )
        model_options.append(
            ft.dropdown.Option(key="error", text="配置中未定义模型", disabled=True)
        )
        ...
        if selected_model_value not in stt_models_config:
            logger.warning(
                f"Configured 'selected_model' ('{selected_model_value}') not found in 'models' list. Falling back."
            )
            ...
            if selected_model_value is None:
                ...
                model_options.append(
                    ft.dropdown.Option(key="error", text="无可用模型", disabled=True)
                )
...
def update_llm_config_ui(
    ...
    if isinstance(preset_dropdown, ft.Dropdown):
        ...
        logger.error(
            "LLM active preset dropdown control not found or invalid in Config Tab."
        )
    ...
    try:
        ...
        update_nested_dict(
            ...
            get_control_value(
                ...
                "dashscope.stt.selected_model",
                str,
                None,  # Default to None if control missing
            ),
        )
        update_nested_dict(
            new_config_data,
        ...
        update_nested_dict(
            ...
            "llm.model",
            get_control_value(
                all_config_controls, "llm.model_dropdown", str, "gpt-3.5-turbo"
            ),  # Use dropdown key
        )
        # --- End get model value ---
        update_nested_dict(
        ...
        update_nested_dict(
            ...
            "llm.max_tokens",
            get_control_value(
                all_config_controls, "llm.max_tokens", int, 256
            ),  # Match default
        )
        ...
        logger.debug(
            f"Saving active_preset_name from dropdown to config: {active_preset_name}"
        )
        ...
        if "llm" in new_config_data and isinstance(new_config_data["llm"], dict):
            ...
            logger.debug(
                "Removed system_prompt and few_shot_examples from data being saved to config.yaml"
            )
...
def reload_config_controls(
        ...
        try:
            ...
            if isinstance(control, ft.Switch):
                ...
                if key == "dashscope.stt.selected_model":
                    new_model_options = []
                    stt_models_config = (
                        reloaded_config_data.get("dashscope", {})
                        .get("stt", {})
                        .get("models", {})
                    )
                    if not stt_models_config:
                        new_model_options.append(
                            ft.dropdown.Option(
                                key="error", text="配置中未定义模型", disabled=True
                            )
                        )
                    else:
                        for model_name, model_info in stt_models_config.items():
                            display_text = model_name
                            supports_translation = model_info.get(
                            ...
                            display_text += ")"
                            new_model_options.append(
                                ft.dropdown.Option(key=model_name, text=display_text)
                            )
                    # Check if options actually changed before updating
                    if str(control.options) != str(
                        new_model_options
                    ):  # Simple string comparison
                        logger.info(
                            f"Reloading options for STT model dropdown ('{key}')."
                        )
                ...
                final_options = (
                    control.options or []
                )  # Use updated options if they changed
                if value is not None and any(opt.key == value for opt in final_options):
                    ...
                    if (
                        value is not None
                    ):  # Log only if a value was expected but invalid
                        logger.warning(
                    ...
                    if key == "audio.device":
                        ...
                        if (
                            control.value is None
                        ):  # If still no valid option (e.g., only error option)
                            control.value = (
                                "error"
                                if any(opt.key == "error" for opt in final_options)
                                else None
                            )
                    else:
                        ...
                        control.value = None
                        logger.warning(
                            f"Setting dropdown '{key}' to None due to invalid value '{value}'."
                        )
    ...
    if "llm.enabled" in all_config_controls:
        all_config_controls["llm.enabled"].value = config_instance.get(
            "llm.enabled", False
        )
    if "llm.api_key" in all_config_controls:
        all_config_controls["llm.api_key"].value = config_instance.get(
            "llm.api_key", ""
        )
    if "llm.base_url" in all_config_controls:
        # Handle None correctly for TextField
        base_url_val = config_instance.get("llm.base_url")
        all_config_controls["llm.base_url"].value = (
            str(base_url_val) if base_url_val is not None else ""
        )
    ...
    if llm_model_dropdown and isinstance(llm_model_dropdown, ft.Dropdown):
        ...
        if any(opt.key == saved_model_value for opt in current_options):
            ...
            logger.debug(
                f"Reload: Set LLM model dropdown value to '{saved_model_value}' (found in options)."
            )
        else:
            ...
            llm_model_dropdown.options = [
                ft.dropdown.Option(
                    key=saved_model_value, text=f"{saved_model_value} (Saved)"
                )
            ] + current_options
    ...
    if "llm.max_tokens" in all_config_controls:
        all_config_controls["llm.max_tokens"].value = str(
            config_instance.get("llm.max_tokens", 256)
        )
    if "llm.extract_final_answer" in all_config_controls:
        all_config_controls["llm.extract_final_answer"].value = config_instance.get(
            "llm.extract_final_answer", False
        )
    if "llm.final_answer_marker" in all_config_controls:
        all_config_controls["llm.final_answer_marker"].value = config_instance.get(
    ...
    if preset_dropdown and isinstance(preset_dropdown, ft.Dropdown):
        ...
        if active_preset_name in preset_names:
            ...
            logger.warning(
                f"Reload: Active preset '{active_preset_name}' not found in options. Selecting 'Default'."
            )
            preset_dropdown.value = "Default" if "Default" in preset_names else None
        logger.debug(
            f"Reload: Set active preset dropdown value to '{preset_dropdown.value}'."
        )

    # --- Update the Preset Tab's display using the callback ---
    # Get the active preset name *again* after potential fallback logic above
    reloaded_active_preset_name = (
        preset_dropdown.value if preset_dropdown else "Default"
    )
    logger.info(
        f"Reloading config: Active LLM preset name is '{reloaded_active_preset_name}'."
    )
...
async def reload_config_handler(
    ...
    update_preset_tab_callback: Optional[
        Callable[[str], None]
    ...
    try:
        ...
        active_preset_label_ctrl_for_reload = (
            None  # TODO: Get this from gui.py partial binding
        )
        # Find the label control (assuming it's passed correctly to this handler's partial)
        # This requires modification in gui.py where the partial is created.
        # Let's modify the signature to expect it.
...
async def open_config_folder_handler(
    ...
    try:
        ...
        if not folder_path.exists() or not folder_path.is_dir():
            logger.error(
                f"Configuration folder path does not exist or is not a directory: {folder_path}"
            )
...
async def fetch_and_update_llm_models_dropdown(
    ...
    try:
        logger.debug(
            f"Fetching models using Base URL: {base_url}, Key: {'*' * (len(api_key) - 4) + api_key[-4:] if api_key else 'None'}"
        )
        client = AsyncOpenAI(api_key=api_key, base_url=base_url)
        models_response = await client.models.list()
        models = sorted(
            [model.id for model in models_response.data], key=str.lower
        ...
        new_options = [
            ft.dropdown.Option(key=model_id, text=model_id) for model_id in models
        ]
        ...
        if current_value in models:
            ...
            logger.info(
                f"Previously selected model '{current_value}' not in fetched list. Selecting '{model_dropdown_ctrl.value}'."
            )
        ...
        logger.error(
            "LLM Authentication Error: Invalid API Key or Base URL.", exc_info=True
        )
        gui_utils.show_error_banner(page, "LLM 认证错误：无效的 API Key 或 Base URL。")
        model_dropdown_ctrl.options = [
            ft.dropdown.Option(key="error", text="认证错误", disabled=True)
        ]
        model_dropdown_ctrl.value = "error"
    except APIConnectionError as e:
        logger.error(
            f"LLM API Connection Error: Could not connect to {base_url or 'default URL'}.",
            exc_info=True,
        )
        gui_utils.show_error_banner(page, f"LLM 连接错误：无法连接到 API 端点。 {e}")
        model_dropdown_ctrl.options = [
            ft.dropdown.Option(key="error", text="连接错误", disabled=True)
        ]
        model_dropdown_ctrl.value = "error"
        ...
        model_dropdown_ctrl.options = [
            ft.dropdown.Option(key="error", text="API 错误", disabled=True)
        ]
        model_dropdown_ctrl.value = "error"
    except Exception as e:
        ...
        model_dropdown_ctrl.options = [
            ft.dropdown.Option(key="error", text="未知错误", disabled=True)
        ]
        model_dropdown_ctrl.value = "error"
    finally:
        # Re-enable button
